module Tinyfb
  class Client
    ...
    def self.raise_client_exception(post_data)
      ...
      if Tinyfb::Client.const_defined?(type)
        raise Tinyfb::Client.const_get(type).new(message)
      else
    ...
    def self.create_from_session_key(session_key, client_id, secret)
      authenticator = Tinyfb::Authenticator.new(client_id, secret, nil)
    ...
    def get_and_map(path,klass=nil,body_args = {})
     ...
     def get_url(path,klass=nil,body_args = {})
        data = self.class.get(api_path(path),:query=>default_params.merge(body_args))
         data
      end
    ...
    def constantize_string(klass)
      klass.is_a?(String) ? Tinyfb.const_get(klass.capitalize) : klass
    end
    ...
    def raise_error_if_necessary(data)
      if data.kind_of?(Hash)
        if data.keys.size == 1 and data["error"]
          ...
          raise Tinyfb::Client.const_get(type).new(message) if Tinyfb::Client.const_defined?(type)
          raise ClientException.new("#{type}: #{message}")
        end
      end
    end
